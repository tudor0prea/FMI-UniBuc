...
class Graf:
    ...
    def succesori(self, nod):
        ...
        for istiva,stiva in enumerate(nod.info):
            ...
            for istiva2,stiva2 in enumerate(copieStive):
                if istiva==istiva2:
                    continue #sa nu il punem pe aceeasi stiva de unde l am luat
    ...
    def calculeaza_h(self, info):
        return 0 #TEMA: de calculat estimatia (nr de blocuri care nu sunt la locul lor)

    def valideaza(self):
        cond1=all( len(self.start)==len(scop) for scop in self.scopuri)
        listaBlocuri = sorted( sum(self.start, start=[]) ) #start-ul lui sum, ca sa nu fie 0
