...
select * from rental;
-- null inseamna ca inca nu e returnata

-- ex. 5 - filmele si copiile ne returnate
select copy_id, title_id from rental
where act_ret_date is NULL;

         -- cate mai sunt disponibile


--select copy_id, title_id 
select title_id,count(*) copii_disponibile,
(select count(*) 
from title_copy where title_id=t.title_id) total_copii
from title_copy t
where (copy_id,title_id) not in 
(select copy_id, title_id 
from rental
where act_ret_date is NULL) group by title_id;


--(select count(*) 
--from title_copy where title_id=t.title_id)
...
BEGIN
    ...
    BEGIN
       ...
       --pozitia 1 
        DBMS_OUTPUT.PUT_LINE(v_client_id||' - valoarea variabilei v_client_id la pozitia 1 ');        
        DBMS_OUTPUT.PUT_LINE(v_client_nume||' - valoarea variabilei v_client id la pozitia 1');
        DBMS_OUTPUT.PUT_LINE(v_nou_client_nume || ' - poz 1');
        DBMS_OUTPUT.PUT_LINE(v_nou_client_id || ' - poz 1');
        
    END;
    v_client_id := ( v_client_id * 12 ) / 10; 
    --pozitia 2 
        -- DBMS_OUTPUT.PUT_LINE(v_client_id||' - valoarea variabilei v_client_id la pozitia 2 ');        
        -- DBMS_OUTPUT.PUT_LINE(v_client_nume||' - valoarea variabilei v_client id la pozitia 2');
       -- DBMS_OUTPUT.PUT_LINE(v_nou_client_nume || ' - poz 2');
      --  DBMS_OUTPUT.PUT_LINE(v_nou_client_id || ' - poz 2');
