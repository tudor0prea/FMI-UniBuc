...
CREATE OR REPLACE FUNCTION f1_ot 
  (v_nume departments.department_name%TYPE )    
RETURN NUMBER IS
    --v_departament employees.department_name%rowtype;
    v_exc exception;
    v_angajati NUMBER;
  ...
  where department_name = v_nume;
    ...
    SELECT count(employee_id) INTO v_angajati 
    FROM   employees
    WHERE  department_id = id_departament;
    
--    if (v_angajati = 0) then raise v_exc;
--    end if;
    RETURN v_angajati;
...
v_nume departments.department_name%TYPE;
BEGIN
    v_nume:='&p_nume_departament';
    
    if(f1_ot(v_nume)!=0) then
    DBMS_OUTPUT.PUT_LINE('In acest departament lucreaza ' || f1_ot(v_nume) || ' angajati.');
    else
    DBMS_OUTPUT.PUT_LINE('In acest departament nu lucreaza nimeni ');
    end if;
EXCEPTION
   -- when v_exc then  DBMS_OUTPUT.PUT_LINE('Nu exista acest departament!');
    WHEN NO_DATA_FOUND THEN
       RAISE_APPLICATION_ERROR (-20000, 'Nu exista acest departament');
    WHEN TOO_MANY_ROWS THEN
...
-- DE LA MELANIA
--Pentru un nume de departament dat de la tast, afisati nr de angajati care lucreaza in acel department.
DECLARE
  v_nume departments.department_name%TYPE; 
  
  
  PROCEDURE calculeaza (v_nume departments.department_name%TYPE )
  IS 
  ...
  BEGIN
    ...
    SELECT count(employee_id) INTO nr_angajati 
    FROM   employees 
    WHERE department_id = v_id;
    
    if (nr_angajati = 0) then
        DBMS_OUTPUT.PUT_LINE('Nu exista angajati in departmanetul dat');
    else 
          DBMS_OUTPUT.PUT_LINE('In acest departament lucreaza ' || nr_angajati || ' angajati.');
    end if;
...
/ --idee : procedura cu in si out
-- sfarsit DE LA MELANIA
