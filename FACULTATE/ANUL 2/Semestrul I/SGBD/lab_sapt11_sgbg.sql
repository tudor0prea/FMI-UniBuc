...
CREATE OR REPLACE PACKAGE pachet1_OT AS
   FUNCTION  f_manager(v_nume VARCHAR, v_prenume VARCHAR) 
        RETURN NUMBER;
    FUNCTION  f_departament(v_nume_dep departments.department_name%TYPE) 
        RETURN NUMBER;
    FUNCTION  f_job(v_nume_job VARCHAR) 
        RETURN NUMBER;
        
END pachet1_ot;
/

CREATE OR REPLACE PACKAGE body pachet1_OT AS
   FUNCTION  f_manager(v_nume VARCHAR, v_prenume VARCHAR) 
        RETURN NUMBER is 
            v_id number;
        BEGIN
            ...
            and last_name = v_prenume;
            
            return v_id;
            
        EXCEPTION
            when NO_DATA_FOUND then 
                DBMS_OUTPUT.PUT_LINE('Nu exista manager/angajat');
   ...
   FUNCTION  f_departament(v_nume_dep departments.department_name%TYPE) 
        RETURN NUMBER IS
            v_cod_dep NUMBER;
        BEGIN
            select department_id into v_cod_dep 
            from departments 
   ...
   FUNCTION  f_job(v_nume_job VARCHAR) 
        ...
        EXCEPTION
            when NO_DATA_FOUND then 
                DBMS_OUTPUT.PUT_LINE('Nu exista acest job');
            when TOO_MANY_ROWS then 
                DBMS_OUTPUT.PUT_LINE('Exista mai multe joburi cu acest nume');
            when OTHERS then
                DBMS_OUTPUT.PUT_LINE('Alta eroare');
        END;
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('test 1: ' || pachet1_ot.f_departament(102));
end;
