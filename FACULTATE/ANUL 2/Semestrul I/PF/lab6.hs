...
doarPozN (M matrice) n = foldr (&&) True (map (foldr (\x rezultat -> (x>0) && rezultat) True) [linie | (L linie) <-matrice, n==length linie] )

        --varianta 2:
doarPozN2 :: Matrice -> Int -> Bool
doarPozN2 (M matrice) n = foldr (&&) True [ all (>0) linie | L linie <- matrice, length linie == n]
--                         ^--  and   --^        
...
testPoz2 = doarPozN (M [L[1,2,-3], L[4,5], L[2,3,6,8], L[8,5,3]]) 3 == False
    ...
    --c)
corect :: Matrice -> Bool
corect (M matrice) =foldr (&&) True [length linie1 == length linie2 | L linie1 <- matrice, L linie2 <- matrice]
